...
ENV VIRTUAL_ENV /opt/sungen-env
ENV PATH $VIRTUAL_ENV/bin:$PATH
RUN python -m venv $VIRTUAL_ENV

# Set the working directory.
WORKDIR /workspaces/sungen/
...
COPY --chown=user:user poetry.lock* pyproject.toml /workspaces/sungen/
RUN mkdir -p /home/user/.cache/pypoetry/ && mkdir -p /home/user/.config/pypoetry/ && \
    mkdir -p src/sungen/ && touch src/sungen/__init__.py && touch README.md
...
COPY --chown=user:user .pre-commit-config.yaml /workspaces/sungen/
...
# Expose the app.
ENTRYPOINT ["/opt/sungen-env/bin/sungen"]
CMD []
